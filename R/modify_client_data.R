...
PDM_url_cols <- PDM_tool %>% filter(type %in% c("audio", "image")) %>% pull(name) %>% c("Household_ID", "Household_Name", "doortag_number")
...
pdm_dt_approved$data <- pdm_dt_approved$data %>% 
  ...
  # mutate(across(all_of(c("doortag_number")), function(x){x=case_when(!is.na(x) ~ "REDACTED", TRUE ~ x)})) %>%
